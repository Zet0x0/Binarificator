...
QString convertMilliseconds(int milliseconds)
{
    ...
    return QString("%0:%1:%2")
        .arg(QString::number(hours).rightJustified(2, '0'),
             ...
             QString::number((milliseconds - hours * 3600000 - minutes * 60000) / 1000)
                 .rightJustified(2, '0'));
}
...
int main(int argc, char **argv)
{
    ...
    QString imageFileName = QFileDialog::getOpenFileName(
        0, "", "",
        QString("Image File (*.%0)").arg(QImageReader::supportedImageFormats().join("; *.")));
    ...
    QString resultFileName = QFileDialog::getSaveFileName(
        0, "", "",
        QString("Image File (*.%0)").arg(QImageWriter::supportedImageFormats().join("; *.")));
    ...
    if (QMessageBox::question(0, "",
                              "Would you like to resize the image to make zeros and ones less "
                              "visible? This could affect performance!") == QMessageBox::Yes)
    {
        image = image.scaled(image.width() * fontMetrics.boundingRect("0").width(),
    ...
    if (QMessageBox::question(
            0, "", "Would you like to change the backround from transparent to something else?") ==
        QMessageBox::Yes)
    {
    ...
    for (int y = 0; y < image.height();
         y += ((fontMetrics.boundingRect("0").height() > fontMetrics.boundingRect("1").height())
                   ? fontMetrics.boundingRect("0").height()
                   : fontMetrics.boundingRect("1").height()))
    {
        for (int x = 0; x < image.width();
             x += ((fontMetrics.boundingRect("0").width() > fontMetrics.boundingRect("1").width())
                       ...
                       : fontMetrics.boundingRect("1").width()))
        {
            ...
            qDebug().noquote() << "Rendering..."
                               << QString("%0%").arg(
                                      QString::number(double(y * image.width() + x) /
                               ...
                               << "|"
                               << QString("X(%0/%1);")
                                      .arg(QString::number(x).rightJustified(
                                               QString::number(image.width()).length(), ' '),
                                           QString::number(image.width()))
                               << QString("Y(%0/%1);")
                                      .arg(QString::number(y).rightJustified(
                                               QString::number(image.height()).length(), ' '),
